...
const  Header =({size,variant,className,isTechnomileLogo, isGovSearchLogo,isSearch,isRightNav, openMenuBar, isFederal, isAdmin, isProfessionalEdition, isProfessional, isStateLocal,isGovSearchProfessional,})=> {
    const [showNotification, SetShowNotification] = useState(false);
    const [showBC, setShowBc] = useState(false);


  return (
    <div className={`${Styles.header}  ${Styles[size]} ${Styles[variant]}  ${className}`}>
         <div className={`${Styles.headerContent} ${Styles.alignBetween} ${Styles.flexWrap} ${Styles.alignItemsCenter}`}>
         <div className={Styles.headerLeft}>
          {isGovSearchLogo &&
            <Text strong="strong7"  className={Styles.headerHeading}>GovSearch</Text>
          }
        ...
        <div className={Styles.headerRight}>
          ...
          {isRightNav &&
            <>
              ...
              <Tooltip title="Currently, Click event is not developed">
                <div>
                  <Icon variant={"gray"} size="icon_xlarge" type={"menu"} className={Styles.headerMenu} />
                </div>
              </Tooltip>
      ...
      {showNotification &&
        <div className={Styles.notificationContent}>
          <div className={Styles.notificationText}>
            ...
            <Text className={Styles.notificationAlert1}><span className={Styles.notificationAlertDark}>Alert:</span> GovSearch will be down for scheduled maintenence on April 20th 2022 at 3:44 pm ET
              till  April 22nd at 1 am ET...</Text>
            <Text className={Styles.notificationMiddleLine}></Text>
            <Text className={`${Styles.notificationAlert2} ${Styles.notificationAlert1}`}><span className={Styles.notificationAlertDark}>Alert:</span> GovSearch will be down for scheduled maintenence on April 20th 2022 at 3:44 pm ET
              till  April 22nd at 1 am ET...</Text>
            <Tooltip title="Currently, Click event is not developed">
              <div>
                <Button variant={"solidPrimary"} className={Styles.notificationAllBtn}>All Alerts</Button>
              </div>
            </Tooltip>
          </div>
        </div>
        ...
        {showBC &&
        <div className={Styles.notificationContent}>
          <div className={`${Styles.notificationBC} ${Styles.notificationText} `}>
            <Text strong="strong7" className={`${Styles.bcList} ${Styles.bcListm15}`}>Account</Text>
            <Text strong="strong7" className={`${Styles.bcList} ${Styles.bcListm15}`}>User Management</Text>
            <Text strong="strong7" className={`${Styles.bcList} ${Styles.bcListm15}`}>Search History</Text>
            <Text strong="strong7" className={`${Styles.bcList} ${Styles.bcListm15}`}>Notification</Text>
            <Text strong="strong7" className={`${Styles.bcList} ${Styles.bcListm15}`}>Help</Text>
            <Text strong="strong7"  className={`${Styles.bcList} ${Styles.bcListm15}`}>Log Out</Text>
          </div>
        </div>
      }
