...
const Layout = ({
  ...
  return (
    <div
      className={`${Styles.layoutMain} ${Styles.scrollBar} ${Styles.scrollWrapper}`}
    >
      {isHeader && (
        <Header
          isSearch={isSearch}
          isFederal={isFederal}
          isGovSearchProfessional={isGovSearchProfessional}
      ...
      {isSidebar && (
        <div className={`${layoutClass} ${Styles.sidebar}`}>
          <Sidebar />
        </div>
      )}
